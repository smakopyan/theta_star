...
class PPOAgent:
    def __init__(self, env):
        ...
        self.epsilon = 0.5 # параметр клиппинга
    ...
    def get_action(self, states):
        ...
        for i in range(self.num_robots):
            ...
            training_logger.info(f"Policy std: {std.numpy().squeeze()}, entropy: {entropy.numpy().squeeze()}")
    ...
    def train(self, max_episodes=500, batch_size=32, num_robots=2):
        ...
        for episode in range(max_episodes):
            ...
            if (episode + 1) % window == 0:
                reward_logger.info(f"Episode {episode+1}: avg_reward/{window} = {avg_reward:.2f}")
